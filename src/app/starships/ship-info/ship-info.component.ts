...
export class ShipInfoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(id) {
      this.starshipSub = this.starshipsService.getStarshipById(id)
      ...
      .subscribe({
        next: (starship: IStarship) => {
          ...
          this.shipInfo = [
            ...
            {label: 'Cost in Credits', text: `₹ ${cost_in_credits}`, cols: 1, rows: 1},
            {label: 'Length', text: length, cols: 1, rows: 1},
            {label: 'Max Atmosphering Speed', text: max_atmosphering_speed, cols: 1, rows: 1},
            {label: 'Crew', text: crew, cols: 1, rows: 1},
            {label: 'Passengers', text: passengers, cols: 1, rows: 1},
            {label: 'Cargo Capacity', text: cargo_capacity, cols: 1, rows: 1},
            {label: 'Consumables', text: consumables, cols: 1, rows: 1},
            {label: 'Hyperdrive Rating', text: hyperdrive_rating, cols: 1, rows: 1},
            {label: 'MGLT', text: MGLT, cols: 1, rows: 1},
            {label: 'Starship Class', text: starship_class, cols: 1, rows: 1}
          ]
          this.shipName = name;
        }
      })
