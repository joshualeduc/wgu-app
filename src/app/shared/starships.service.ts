...
export class StarshipsService {
  ...
  getStarships(): Observable<IStarship[]> {
    return this.http.get<IStarship[]>(this.starshipsUrl).pipe(
      catchError(this.handleError)
    )
  }

  getStarshipById(id: string): Observable<IStarship> {
    return this.http.get<IStarship>(`${this.starshipIdUrl}${id}/`).pipe(
      catchError(this.handleError)
    )
  }

  getStarshipsWithPilots(): Observable<IStarship[]> {
    return this.http.get<IStarship[]>(this.starshipsWithPilotsUrl).pipe(
      catchError(this.handleError)
    )
  }
